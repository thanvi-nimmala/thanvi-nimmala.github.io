...
(() => {
  ...
  // Passthrough types skip canvas re-encode: animated GIFs would lose every
  // frame but the first, and video has no canvas path at all.
  const PASSTHROUGH = ['image/gif', 'video/mp4', 'video/webm', 'video/quicktime'];
  ...
  async function toDataUrl(file, targetW) {
    if (PASSTHROUGH.indexOf(file.type) >= 0) return readAsDataUrl(file);
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <video class="vid" part="video" playsinline autoplay muted loop style="display:none"></video>' +
      ...
      this._vid = root.querySelector('.frame video');
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Drop a PNG, JPEG, WebP, AVIF, GIF, MP4, WebM or MOV file.');
        return;
      }
      if (PASSTHROUGH.indexOf(file.type) >= 0 && file.size > MAX_PASSTHROUGH_BYTES) {
        this._setError('That file is over 12MB — compress it and try again.');
        return;
      }
    ...
    _render() {
      ...
      if (url && !attrError) {
        ...
        if (isVideo) {
          if (this._vid.getAttribute('src') !== url) this._vid.src = url;
        ...
        if (this._vid) { this._vid.style.display = 'none'; this._vid.removeAttribute('src'); }
